...
function FallbackMap({ selectedPlace }: { selectedPlace: PlaceResult | null }) {
  return (
    <div className="map-fallback">
      ...
      {selectedPlace ? <div className="pin p1">선택</div> : <><div className="pin p1">잠실</div><div className="pin p2">과천</div><div className="pin p3">분당</div></>}
      <div className="route-line" />
    </div>
  );
}

function useKakaoMap(containerRef: React.RefObject<HTMLDivElement | null>, candidates: Candidate[], selectedPlace: PlaceResult | null) {
  ...
  useEffect(() => {
    ...
    const drawMap = () => {
      try {
        ...
        const companyPos = new window.kakao.maps.LatLng(companyAnchor.lat, companyAnchor.lng);
        new window.kakao.maps.Marker({ map, position: companyPos, title: companyAnchor.name });
        bounds.extend(companyPos);

        const targets = selectedPlace
          ? [{ name: selectedPlace.placeName, region: selectedPlace.categoryName, lat: selectedPlace.lat, lng: selectedPlace.lng, grade: '선택', score: 0, commute: 0 }]
          : candidates.slice(0, 6);

        targets.forEach((target) => {
          const pos = new window.kakao.maps.LatLng(target.lat, target.lng);
          const marker = new window.kakao.maps.Marker({ map, position: pos, title: target.name });
          const info = new window.kakao.maps.InfoWindow({
            content: `<div style="padding:8px 10px;font-size:12px;font-weight:700;white-space:nowrap">${target.name}<br/><span style="font-weight:400;color:#64748b">${target.region ?? ''}</span></div>`,
          });
    ...
    const oldScript = document.querySelector<HTMLScriptElement>('script[data-kakao-map="true"]');
  ...
  }, [containerRef, candidates, selectedPlace]);

  return { status, origin, keyDetected };
}
...
export default function Finder() {
  ...
  const top = selectedPlace ? { name: selectedPlace.placeName, grade: '선택', score: '', type: selectedPlace.categoryName } : activeCandidates[0];
  const map = useKakaoMap(mapRef, activeCandidates, selectedPlace);
